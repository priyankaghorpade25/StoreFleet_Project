...
userSchema.pre("save", async function (next) {
  ...
  if (this.isModified('password') || this.isNew) {
    try {
      ...
      this.password = hashedPassword; // Set the password to the hashed version
      next(); // Proceed to save the user
    } catch (error) {
      next(error); // Pass any errors to the next middleware
    }
  } else {
    next(); // If the password is not modified, proceed to save the user
  } 
});
...
userSchema.methods.comparePassword = async function (password) {
  ...
  console.log("Inside comparePassword function")
  console.log(password,this.password);

  const result=await bcrypt.compare(password, this.password);
  
  console.log("Password match or not result",result);
  return result;
};
